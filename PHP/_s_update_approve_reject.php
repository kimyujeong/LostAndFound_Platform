<?php 
    ...
    if( (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit'])) || $android )
    {
        ...
        else if(empty($reject_detail)){
            $errMSG = "reject_detail 입력하세요.";
        }
        ...
        if(!isset($errMSG)) // 이름과 나라 모두 입력이 되었다면 
        {
            try{
                ...
                $stmt = $con->prepare('UPDATE inquiry SET processing_state=:processing_state, flag=:flag, reject_detail=:reject_detail WHERE customer_num=:customer_num and lost_num=:lost_num');
                ...
                $stmt->bindParam(':reject_detail', $reject_detail);
